...
proc display_proj {} {
    ...
    global SHELL_MODE
    global BUILD_FLOW
    global BUILD_STRATEGY
    global BUILD_STRATEGY_FILE
    global PLACE_DIRECT
    global PHY_OPT_DIRECT
    global ROUTE_DIRECT
    global STRATEGY_SYNTH_OPTIONS
    global STRATEGY_SYNTH_DIRECTIVE
    global STRATEGY_OPT_OPTIONS
    global STRATEGY_OPT_DIRECTIVE
    global STRATEGY_PLACE_OPTIONS
    global STRATEGY_PHYS_OPT
    global STRATEGY_PHYS_OPTIONS
    global STRATEGY_ROUTE_OPTIONS
    global STRATEGY_ROUTE_PHYS_OPT
    global STRATEGY_POST_PHYS_OPTIONS
    global STRATEGY_POST_PHYS_DIRECTIVE
    global TAG
    ...
    strategy_file    : $BUILD_STRATEGY_FILE
    place_direct     : $PLACE_DIRECT
    phy_opt_direct   : $PHY_OPT_DIRECT
    route_direct     : $ROUTE_DIRECT
    synth_options    : $STRATEGY_SYNTH_OPTIONS
    synth_directive  : $STRATEGY_SYNTH_DIRECTIVE
    opt_options      : $STRATEGY_OPT_OPTIONS
    opt_directive    : $STRATEGY_OPT_DIRECTIVE
    place_options    : $STRATEGY_PLACE_OPTIONS
    phys_opt         : $STRATEGY_PHYS_OPT
    phys_options     : $STRATEGY_PHYS_OPTIONS
    route_options    : $STRATEGY_ROUTE_OPTIONS
    route_phys_opt   : $STRATEGY_ROUTE_PHYS_OPT
    post_phys_opts   : $STRATEGY_POST_PHYS_OPTIONS
    post_phys_direct : $STRATEGY_POST_PHYS_DIRECTIVE
    build_tag        : $TAG
    ==================================================
    "
    puts "$rpt"
...
set BUILD_STRATEGY "LEGACY_FIXED"
set BUILD_STRATEGY_FILE ""

set STRATEGY_SYNTH_OPTIONS "-keep_equivalent_registers"
set STRATEGY_SYNTH_DIRECTIVE ""

set STRATEGY_OPT_OPTIONS ""
set STRATEGY_OPT_DIRECTIVE ""

set STRATEGY_PLACE_OPTIONS ""
set STRATEGY_PHYS_OPT 1
set STRATEGY_PHYS_OPTIONS ""
set STRATEGY_ROUTE_OPTIONS "-tns_cleanup"
set STRATEGY_ROUTE_PHYS_OPT 0
set STRATEGY_POST_PHYS_OPTIONS ""
set STRATEGY_POST_PHYS_DIRECTIVE ""

if { [info exists ::env(BUILD_STRATEGY)] } {
    set BUILD_STRATEGY $::env(BUILD_STRATEGY)
    set BUILD_STRATEGY_FILE "${HDK_SHELL_DIR}/build/scripts/strategy_${BUILD_STRATEGY}.tcl"
    if {![file exists $BUILD_STRATEGY_FILE]} {
        print "ERROR: BUILD_STRATEGY requested missing file ${BUILD_STRATEGY_FILE}"
        exit 1
    }

    source $BUILD_STRATEGY_FILE

    set STRATEGY_SYNTH_OPTIONS $synth_options
    set STRATEGY_SYNTH_DIRECTIVE $synth_directive

    set STRATEGY_OPT_OPTIONS $opt_options
    set STRATEGY_OPT_DIRECTIVE $opt_directive

    set STRATEGY_PLACE_OPTIONS $place_options
    if {$place_directive ne ""} {
        set PLACE_DIRECT $place_directive
    }

    set STRATEGY_PHYS_OPT $phys_opt
    set STRATEGY_PHYS_OPTIONS $phys_options
    if {$phys_directive ne ""} {
        set PHY_OPT_DIRECT $phys_directive
    }

    set STRATEGY_ROUTE_OPTIONS $route_options
    if {$route_directive ne ""} {
        set ROUTE_DIRECT $route_directive
    }
    set STRATEGY_ROUTE_PHYS_OPT $route_phys_opt
    set STRATEGY_POST_PHYS_OPTIONS $post_phys_options
    set STRATEGY_POST_PHYS_DIRECTIVE $post_phys_directive
}
